...
private:
    ...
    void iteration(const ros::TimerEvent& e)
    {
        ...
        // update robot state from tf
        this->getPose();
        //std::cout << curr_pose << std::endl;
        

        //std::cout << m_goal.pose.position << std::endl;
        switch(m_state)
        {
        ...
        case Automatic:
            {
                ...
                msg.linear.y = m_pidY.update(0, targetDrone.pose.position.y);
                msg.linear.z = m_pidZ.update(0, targetDrone.pose.position.z);
    ...
    void getPose()
    {
        ...
        m_listener.lookupTransform(m_worldFrame, m_frame, ros::Time(0), trans);
        curr_pose.position.x = trans.getOrigin().x();
        curr_pose.position.y = trans.getOrigin().y();
        curr_pose.position.z = trans.getOrigin().z();
        curr_pose.orientation.x = trans.getRotation().x();        
        curr_pose.orientation.y = trans.getRotation().y();        
