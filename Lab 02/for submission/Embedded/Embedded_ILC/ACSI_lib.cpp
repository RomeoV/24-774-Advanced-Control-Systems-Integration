...
float u_ilc[numTimesteps];
float reference[numTimesteps];
float response[numTimesteps];
...
void recordReferenceAndResponse(float tsps){
  ...
  for (unsigned int timestep = current_timestep; timestep < current_timestep+4 && timestep < numTimesteps; timestep++) {
...
void setControlInput() {
  ...
  if (currentPhase == Phase::ILC_STEP) {
    recordReferenceAndResponse(tsps);
    theta_ref = sin(1.5*M_PI*tsps);
    if (tsps >= 10.0 && currentILCIter < maxILCIterations) {
      currentPhase = Phase::RECENTER;

      timeAtPhaseStart = micros();
    }
    else if (currentILCIter >= maxILCIterations) {
      currentPhase = Phase::END;
    }
  }

  else if (currentPhase == Phase::RECENTER) {
    theta_ref = 0.;
    if (tsps >= 3.0) {
      currentPhase = Phase::ILC_STEP;
      
      computeCurrentError();
      updateILCCommands();
      Serial.print(currentILCIter);Serial.print(",");Serial.println(error[currentILCIter]);
      alpha *= 0.95;
  ...
  if (currentPhase == Phase::ILC_STEP || currentPhase == Phase::RECENTER) {
    ...
    if (tsps < 0.13) motorVoltage = 0.;
    else if (tsps >= 0.13 && tsps < 0.17) motorVoltage = 16.;
    else if (tsps >= 0.17 && tsps < 0.5) motorVoltage = 0.;
    else if (tsps >= 0.5 && tsps < 0.55) motorVoltage = -20.;
    else if (tsps >= 0.55 && tsps < 0.62) motorVoltage = 0.;
    else if (tsps >= 0.62 && tsps < 0.68) motorVoltage = 15.5;
    else if (tsps >= 0.68 || fabs(alpha) < 0.4) {
      currentPhase = Phase::RECENTER;
      timeAtPhaseStart = micros();
    }
    else motorVoltage = 0.;
...
void updateILCCommands() {
  for (unsigned ts = 0; ts < numTimesteps-1; ts++) {
    u_ilc[ts] += learning_rate*(reference[ts+1]-response[ts+1]);
  }
}
...
void computeCurrentError() {
  ...
  for (unsigned ts = 0; ts < numTimesteps; ts++) {
    median_value+=(reference[ts]-response[ts]);
  }
  median_value /= numTimesteps;
  ...
  standard_deviation = sqrt(inner_sum/numTimesteps);
