...
float Pitch_deg=0;
float Yaw_deg=0;
float internal_state[5] = {0,0,0,0,0};
//Controller State Space

float A_siso_loop[5][5] = {
{2.6375,-1.1539,0.67032,0,0},
{2,0,0,0,0},
{0,0.5,0,0,0},
{0,0,0,1.6375,-0.67032},
{0,0,0,1,0}};

float B_siso_loop[5][2] = {
{64,0},
{0,0},
{0,0},
{0,32},
...
float C_siso_loop[2][5] = {
{20.617,-20.833,21.048,0,0},
{0,0,0,20.75,-21.148}};

float D_siso_loop[2][2] = {
{871.84,0},
{0,438.22}};


float A_dido_loop[5][5] = {
{2.6375,-1.1539,0.67032,0,0},
{2,0,0,0,0},
{0,0.5,0,0,0},
{0,0,0,1.6375,-0.67032},
...
float C_dido_loop[2][5] = {
{12.335,-12.464,12.592,-11.85,12.077},
{15.138,-15.296,15.454,6.207,-6.3262}};

float D_dido_loop[2][2] = {
{260.8,-250.26},
{320.07,131.09}};
...
//    }
  ...
  //  float encoder1Deg = (float)encoder1 * (360.0 / 2048.0);  // motor 1 angle
    ...
    StateX[1]=YawRad;
    /*
      //**** Z   1st order derrivative filter   start *******************
    float Pitch_transforn=PitchRad;
    ...
    */ 
    ...
    //doSisoLoopShapingController(Error[0],Error[1]);
    doDidoLoopShapingController(Error[0],Error[1]);
 ...
 //
      float sat_limit = 15.0;
      if (motor0Voltage > sat_limit) {
        motor0Voltage = sat_limit;
      }
      else if (motor0Voltage < -sat_limit) {
        motor0Voltage = -sat_limit;
      }
// set the saturation limit to +/- 15V for Motor1
      if (motor1Voltage > sat_limit) {
        motor1Voltage = sat_limit;
      }
      else if (motor1Voltage < -sat_limit) {
        motor1Voltage = -sat_limit;
      }
...
/*
        ...
        Serial.print(StateX[1]); Serial.println("]");
